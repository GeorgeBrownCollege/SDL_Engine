...
void TextureManager::draw(
	const std::string & id, const int x, const int y, const double angle,
...
void TextureManager::drawFrame(
	const std::string & id, const int x, const int y, const int frame_width,
	const int frame_height, int &current_row, int &current_frame, int frame_number,
	int row_number, float speed_factor, const double angle, const int alpha,
...
void TextureManager::animateFrames(
	int frame_width, int frame_height, const int frame_number,
	const int row_number, const float speed_factor, int &current_frame, int &current_row) {
...
void TextureManager::playAnimation(
	...
	const auto totalFrames = animation.m_frames.size();
	const int animationRate = round(totalFrames / 2 / speed_factor);

	if (totalFrames > 1) {
		if (TheGame::Instance()->getFrames() % animationRate == 0) {
			animation.m_currFrame++;
			if (animation.m_currFrame > totalFrames - 1) {
				animation.m_currFrame = 0;
			}
		}
	}
	...
	// frame_height size
	const auto textureWidth = animation.m_frames[animation.m_currFrame].w;
	const auto textureHeight = animation.m_frames[animation.m_currFrame].h;

	// starting point of the where we are looking
	srcRect.x = animation.m_frames[animation.m_currFrame].x;
	srcRect.y = animation.m_frames[animation.m_currFrame].y;

	srcRect.w = animation.m_frames[animation.m_currFrame].w;
	srcRect.h = animation.m_frames[animation.m_currFrame].h;
